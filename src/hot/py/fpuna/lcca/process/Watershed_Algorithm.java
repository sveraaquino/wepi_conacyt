 ...
 * 
 * @Article{Vincent/Soille:1991, author = "Lee Vincent and Pierre Soille", year = "1991", keywords = "IMAGE-PROC SKELETON SEGMENTATION GIS", institution = "Harvard/Paris+Louvain", title =
 *                               "Watersheds in digital spaces: An efficient algorithm based on immersion simulations", journal = "IEEE PAMI, 1991", volume = "13", number = "6", pages = "583--598", annote = "Watershed
 *                               lines (e.g. the continental divide) mark the boundaries of catchment regions in a topographical map. The height of a point on this map can have a direct correlation to its pixel
 *                               intensity. WIth this analogy, the morphological operations of closing (or opening) can be understood as smoothing the ridges (or filling in the valleys). Develops a new algorithm for
 *                               obtaining the watershed lines in a graph, and then uses this in developing a new segmentation approach based on the {"}depth of immersion{"}.", }
 * 
 *                               A review of Watershed algorithms can be found at : http://www.cs.rug.nl/~roe/publications/parwshed.pdf
 * 
 * @Article{RoeMei00, author = "Roerdink and Meijster", title = "The Watershed Transform: Definitions, Algorithms and Parallelization Strategies", journal = "FUNDINF: Fundamenta Informatica", volume = "41", publisher =
 *                    "IOS Press", year = "2000", }
 **/
...
public class Watershed_Algorithm implements PlugInFilter {
		...
		for (int h = HMIN; h < HMAX; h++) /* Geodesic SKIZ of level h-1 inside level h */{
			...
			while (true) /** extend basins **/
			{
				...
				for (int i = 0; i < neighbours.size(); i++) /* Labelling p by inspecting neighbours */{
					...
					/*
					 * Original algorithm : if( (q.getDistance() < curdist) && (q.getLabel()>0 || q.isLabelWSHED()) ) {
					 */
	...
	public ImageProcessor wepi_run(ImageProcessor ip) {
		...
		return new BinaryProcessor((ByteProcessor) outputImage);
	}
...
class WatershedPixel implements Comparable {
	...
	public void addNeighbour(WatershedPixel neighbour) {
		/*
		 * IJ.write("In Pixel, adding :"); IJ.write(""+neighbour); IJ.write("Add done");
		 */
