...
public class Lipschitz implements PlugInFilter {
	...
	void showAbout() {
		IJ.showMessage("About 2D Lipschitz filter", "version " + Lipschitz_Version + " (" + Lipschitz_Date + ")\n" + "Mikulas Stencel, Jiri Janacek GPL2\n"
	...
	public void Lipschitz2D(ImageProcessor ip) {
		...
		for (int ii = 0; ii < m_channels; ii++) {
			for (int ij = 0; ij < ImageHeight * ImageWidth; ij++) {
				if (m_TopHat) {
					tmpBytePixels[ii][ij] = (m_Down ? (byte) (srcPixels[ii][ij] - destPixels[ii][ij] + 255) : (byte) (destPixels[ii][ij] - srcPixels[ii][ij]));
				} else {
					if (m_short) {
	...
	public void runLipschitz(ImageProcessor ip) {
		...
		imp2 = new ImagePlus(m_imp.getShortTitle() + " Filtered (Lipschitz) Slope:" + m_Slope + " " + ((m_Down) ? " -Down" : " ") + " " + ((m_TopHat) ? " -TopHat" : " ")
				+ ((breaked) ? " -INTERUPTED" : ""), m_stack_out);
		this.m_imp = imp2;
		// imp2.show();
		// imp2.updateAndDraw();
		// IJ.showProgress(1.0);
	...
	public boolean wepi_setup(Object... params) {
		...
		if (!d.isInstance(params[0]))
			return false;

		m_Slope = (Double) params[0];
		if (m_Slope <= 0)
			m_Slope = 10;

		m_Down = (Boolean) params[1];
		m_TopHat = (Boolean) params[2];
		return true;
	} // end of 'getUserParams' method

	public ImagePlus wepi_run(ImageProcessor ip, Object... params) {
		wepi_setup(params);
		this.m_imp = new ImagePlus("Lipschitz", ip);
		m_stack = m_imp.getStack();
		m_scount = m_stack.getSize();
		m_roi = ip.getRoi();
		ImageHeight = ip.getHeight();
