...
public class Color_Dilate implements PlugInFilter {
	...
	public int[] max(float[] weight, Pixel[] SE, Pixel p) {

		int u, v;
		float t = 0.0f, max = -1.0f;
		int cLength = channel.length;
		int[] idx = new int[cLength];
		int[] pixel = new int[cLength];
		int[] maxP = new int[cLength];
		boolean findMaxPixel = true;
		int j = 0;
		...
		for (int i = 0; i < SE.length; i++) {
			...
			if (t == max && !iqualPixels(maxP, pixel)) {
	...
	public void run(ImageProcessor ip) {
		...
		for (int u = 1; u < iPr.getWidth() - 1; u++) {
			for (int v = 1; v < iPr.getHeight() - 1; v++) {
				...
				maxP = max(realWeight, seFour, new Pixel(u, v));
				iPr.putPixel(u, v, maxP);
			}
		}

//		imagePl.setProcessor(iPr);
		// imagePl.show();
	}
