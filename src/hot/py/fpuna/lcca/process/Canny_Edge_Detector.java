 ...
 * 
 * <p>
 * This class provides a configurable implementation of the Canny edge detection algorithm. This classic algorithm has a number of shortcomings, but remains an effective tool in many scenarios. <em>This class is designed
...
public class Canny_Edge_Detector implements PlugIn {
	...
	public ImageProcessor wepi_run(ImageProcessor imp, Object... objects) {
		sourceImage = new ImagePlus("img", imp);
		...
		if (!Prefs.blackBackground)
			ip.invertLut();
		return new BinaryProcessor((ByteProcessor) ip);
	}
	...
	/**
	 * Sets the low threshold for hysteresis. Suitable values for this parameter must be determined experimentally for each application. It is nonsensical (though not prohibited) for this value to exceed the high
	 * threshold value.
	 * 
	 * @param threshold
	...
	/**
	 * Sets the high threshold for hysteresis. Suitable values for this parameter must be determined experimentally for each application. It is nonsensical (though not prohibited) for this value to be less than the low
	 * threshold value.
	 * 
	 * @param threshold
	...
	private void computeGradients(float kernelRadius, int kernelWidth) {
		...
		for (int x = initX; x < maxX; x++) {
			for (int y = initY; y < maxY; y += width) {
				...
				float tmp;
				/*
				 * An explanation of what's happening here, for those who want to understand the source: This performs the "non-maximal supression" phase of the Canny edge detection in which we need to compare the
				 * gradient magnitude to that in the direction of the gradient; only if the value is a local maximum do we consider the point as an edge candidate.
				 * 
				 * We need to break the comparison into a number of different cases depending on the gradient direction so that the appropriate values can be used. To avoid computing the gradient direction, we use two
				 * simple comparisons: first we check that the partial derivatives have the same sign (1) and then we check which is larger (2). As a consequence, we have reduced the problem to one of four identical
				 * cases that each test the central gradient magnitude against the values at two points with 'identical support'; what this means is that the geometry required to accurately interpolate the magnitude of
				 * gradient function at those points has an identical geometry (upto right-angled-rotation/reflection).
				 * 
				 * When comparing the central gradient to the two interpolated values, we avoid performing any divisions by multiplying both sides of each inequality by the greater of the two partial derivatives. The
				 * common comparand is stored in a temporary variable (3) and reused in the mirror case (4).
				 */
