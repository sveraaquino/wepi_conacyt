...
/**
 * This class implements the FFT, Inverse FFT and Redisplay Power Spectrum commands in the Process/FFT submenu. It is based on Arlo Reeves' Pascal implementation of the Fast
 * Hartley Transform from NIH Image (http://imagej.nih.gov/ij/docs/ImageFFT/). The Fast Hartley Transform was restricted by U.S. Patent No. 4,646,256, but was placed in the public
...
public class FFT implements PlugIn, Measurements {
	...
	public ImageProcessor wepi_run(String arg, ImageProcessor ip) {
		imp = new ImagePlus("img", ip);
		FHT fht = newFHT(ip);
		fht.transform();
		return fht.getPowerSpectrum();
	}
	...
	static void smooth(ImageProcessor ip) {
		...
		for (int y = 0; y < n; y++) {
			...
			for (int x = 0; x < n; x++) {
				int sum = (pixels2[offset1 + iMinus[x]] & 255) + (pixels2[offset1 + x] & 255) + (pixels2[offset1 + iPlus[x]] & 255) + (pixels2[offset2 + iMinus[x]] & 255)
						+ (pixels2[offset2 + x] & 255) + (pixels2[offset2 + iPlus[x]] & 255) + (pixels2[offset3 + iMinus[x]] & 255) + (pixels2[offset3 + x] & 255)
