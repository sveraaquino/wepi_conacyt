...
public class Deinterlace implements PlugIn {
	static final String[] methods = { "Even field only", "Odd field only", "Double: even then odd", "Double: odd then even" };
	static String method = methods[0];

	public boolean wepi_setup(Object... objects) {
		method = methods[(Integer) objects[0]];
		return true;
	}

	public ImageProcessor wepi_run(ImageProcessor ip) {
		ImagePlus imp = new ImagePlus("img", ip.duplicate());
